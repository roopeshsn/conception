--- 
title: Create Linux Command Helper
slug: linux-cmd-helper
createdAt: Tue Oct 25 2022 16:57:09 GMT+0530 (India Standard Time)
category: app-ideas
summary: A command helper api to search linux commands
tags: java, kotlin, javascript, swift
author: Vishal Yadav
---

# Create Linux Command Helper

## A command helper api to search linux commands

Create an app to search linux commands to install and learn about various packages.

App should be cross-platform to reach various device users.

App should also give alternative commands to install a single package and also describe which
command to choose in which case.

App should suggest only 2 alternatives to single command to avoid confusion.

App search results should have preference from most commands to least used.